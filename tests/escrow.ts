...
const getRandomBigNumber = () => new BN(randomBytes(8));
...
describe("escrow", () => {
  ...
  before(
    "Creates Alice and Bob's accounts, 2 token mints, and ATAs for both tokens for both users",
    async () => {
  ...
  it("Transfer tokens from Alice to vault", async () => {
  ...
  it("Transfer token from vault to Bob, and transfer token from Bob to Alice at the same time", async () => {
    const txSig = await program.methods
      .takeOffer()
      .accounts({ ...accounts })
      .signers([bob])
